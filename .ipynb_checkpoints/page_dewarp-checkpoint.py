...
def resize_to_screen(src, maxw=1280, maxh=700, copy=False):
    ...
    if scl > 1.0:
        inv_scl = 1.0/scl
        img = cv2.resize(src, (0, 0), None, inv_scl, inv_scl, cv2.INTER_CUBIC)
    elif copy:
        img = src.copy()
    else:
        img = src
...
def get_mask(name, small, pagemask, masktype):
    ...
    if masktype == 'text':
        ...
        if DEBUG_LEVEL >= 3:
            debug_show(name, 0.8, 'eroded', mask)

        mask = cv2.dilate(mask, box(8, 2))

        if DEBUG_LEVEL >= 3:
            debug_show(name, 0.9, 'dilated', mask)
    else:
...
def remap_image(name, img, small, page_dims, params):
    ...
    ori_remapped = cv2.remap(img, image_x_coords, image_y_coords,
                         cv2.INTER_CUBIC,
                         None, cv2.BORDER_REPLICATE)
    ...
    debug_show(name, 7, 'final', ori_remapped)

    return threshfile, ori_remapped


def processing(img, basename):    
#     small = resize_to_screen(img, 2000, 2000)
    small = img
...
def main():
    ...
    for imgfile in sys.argv[1:]:
        ...
        _, outfile = processing(img, basename)
        outfile = cv2.rotate(outfile, cv2.ROTATE_90_CLOCKWISE)
        _, outfile2 = processing(outfile, basename)
